...
const LoginScreen = () => {
  ...
  const fields: FormField = {
    ...
    ButtonStyles:
      "w-full max-w-[80%] md:max-w-[40%] py-3 px-6 border-2 rounded-3xl mt-2 transition duration-500 ease-in-out text-white bg-dbbluedeep hover:text-opacity-80 hover:bg-opacity-60 focus:outline-none focus:border-slate-300 mb-2",
    InputStyles:
      "max-w-full w-full p-4 rounded-3xl placeholder:text-sm placeholder:font-medium focused:border-none focused:outline-none outline-none focused-within:outline-none text-center text-black",
    FormStyles:
  ...
  const handleSubmit: OtherFormTypes<InitialValuesType>["OnSubmit"] = (
    ...
    axios
      ...
      .then(() => {
        ...
        window.location.href = "/success/passauth";
      })
      .catch((err) => toast.error("Failed Request" + err));
    formikHelpers.resetForm();
  ...
  const { Layout, Center } = LoginScreenStyles;
  return (
    <>
      <Layout>
        <Center>
