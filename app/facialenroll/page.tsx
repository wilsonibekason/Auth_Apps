...
import axios from "@/utils/baseUrl"; // Import axios
...
const FacialRecognition: React.FC = () => {
  ...
  const captureFace = async () => {
    try {
      ...
      if (context) {
        ...
        // Send the captured image to the server for enrollment or authentication
  ...
  const sendImageToServer = async (imageData: string) => {
    try {
      // Send either for enrollment or authentication based on the mode
      const response = await axios.post(facialUrlEndPoint, {
        image: imageData,
        userId: userId, // Include userId if needed
        action: "enroll", // or "authenticate" based on your needs
      });
        ...
        <>
          <div className="flex flex-col space-y-5 justify-center items-center h-screen">
            <p className="text-slate-700 text-2xl md:text-5xl font-semibold">
              Enter your UserId to Capture Your Facial Image
            </p>
            <input
              type="text"
              name="userId"
